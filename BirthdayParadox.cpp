...
bool BirthdayParadox::hasDuplicates(const std::vector<int> & birthdays) {
    std::set<int> uniqueBirthdays(birthdays.begin(), birthdays.end());
    return (uniqueBirthdays.size() != birthdays.size());
}

std::vector<int> BirthdayParadox::generateNumbers(const int popSize) {
    std::vector<int> list;
...
void BirthdayParadox::simulate(const int samples, const std::vector<int> & popList, BaseListener & listener) {
